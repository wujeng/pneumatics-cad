...
*/
 ...
 //   ESystem esys=null;
  ...
  private void drawPressure(Graphics2D g2,float p, float f,int x,int y,boolean star,float QC,boolean isp, boolean isd,boolean iss)
   {
/*
 //   g2.setFont(newFont);
...
//-----------
  ...
  {StringBuffer sb=new StringBuffer();
   ...
   for(int i=0;i<lineArray.size();i++)
    {lin=(Line) lineArray.get(i);
     ...
     if(in1 && in2)
      {sb.append(Integer.toString(PneumaticsCAD.Data_Line)+" "+Integer.toString(bi1)+" "+Integer.toString(ei1)+" "+
    	...
    	for(int j=0;j<valveArray.size();j++)
         {if(valveArray.get(j) instanceof PneumaticElement)
            {valve=(PneumaticElement) valveArray.get(j);
             ...
             if(elei>=0) sb.append(Integer.toString(PneumaticsCAD.Data_Limitswitch)+" "+Integer.toString(i)+" "+Integer.toString(elei)+" "+
 ...
 public String saveELimitswitch(Object[] objs)
  ...
  public String save(Object[] objs)  // for saving file and block
   {StringBuffer sb=new StringBuffer();
    for(int i=0;i<objs.length;i++)
...
//-------------   
  ...
  {StringTokenizer token=new StringTokenizer(str);
    ...
    if(dtype!=PneumaticsCAD.Data_Line) return;
    Line lin=null;
    ...
    try
     {e1=Integer.parseInt(token.nextToken());
      ...
      {pneumaticscad.setStatus(Config.getString("PneumaticPanel.lineformaterror"));return;}
     lin=new Line();
...
public void openLimitswitch(String str)
 {	
	StringTokenizer token=new StringTokenizer(str);
    int dtype=Integer.parseInt(token.nextToken());
    if(dtype!=PneumaticsCAD.Data_Limitswitch) return;
    int e1=0,p1=0,e2=0;
    try
     {e1=Integer.parseInt(token.nextToken());
       p1=Integer.parseInt(token.nextToken());
       ...
       {pneumaticscad.setStatus(Config.getString("PneumaticPanel.limitswitchformaterror"));return;}
  ...
  public void open(String line) throws Exception
   {
     ...
     switch(dtype)
      {case PneumaticsCAD.Data_Element: 
    	 PneumaticElement ele=PneumaticElement.read(line, pneumaticscad);
//System.out.println("pneumaticPanel.open(...Element.read");         
     //    if(!WebLadderCAD.checkLimit(ele.getModelType(),ele.getModelName())) return;
         tempElementArray.add(ele);
         add(ele);
         groupArray.add(ele);
         ele.setGrouped(true);
         break;
       case PneumaticsCAD.Data_Line: openLine(line);break;
       case PneumaticsCAD.Data_Limitswitch: openLimitswitch(line);break;
 //      case PneumaticsCAD.Data_ELimitswitch: openELimitswitch(line);break;
      }
...
/*   
   private class selectLineCommand extends Command
    {Line line;
	public selectLineCommand(Object ele,Line line)
     {super(ele,"Select Pneumatic Pipe");
      this.line=line;
     }
    public void undo()
     {selectedLine=null;
...
//       if(net &&  pneumatics.allClient.connected) pneumatics.writeEvent(new PneumaticsEvent(PneumaticsEvent.S_pneumaticPanel,"mousePressed",e,null));
      }
   }
...
/*   
   private class groupCommand extends Command
    {ArrayList newgroup;
     ArrayList oldgroup;
	public groupCommand(Object ele,ArrayList og,ArrayList ng)
     {super(ele,"Group");
      newgroup=ng;
      oldgroup=og;
     }
    public void undo()
     {Element element;
	  for(int i=0;i<newgroup.size();i++)
       {element=(Element) newgroup.get(i);
      ...
      if(left)
       {if(groupMove)
         {ArrayList garray=new ArrayList(groupArray);
          ...
          boolean modified=pneumaticscad.getModified();
          pneumaticscad.setModified(true);
          pneumaticscad.addCommand(new moveGroupCommand(this,modified,garray,delx,dely));
/*	      
          gmovePos.x=ex;
          gmovePos.y=ey;
          gmovePosf1.x+=delx;
   ...
   private class moveGroupCommand extends Command
    ...
    public void undo()
     {PneumaticElement element=null;
      ...
      pneumaticscad.setModified(modified);
      repaint();
     }
    public void redo()
     {PneumaticElement element=null;
...
//System.err.println(kcode);
  ...
  public void pasteBoardTo()
   {boolean modified=pneumaticscad.getModified();
    ...
    tw.com.justiot.pneumatics.pneumaticelement.PneumaticElement elementpaste=null;
    Object obj=null;
    for(int i=0;i<boardArray.size();i++)
     {be=(BoardElement) boardArray.get(i);
      try
       {obj=Creater.instanceElement(be.modelType,be.modelName,pneumaticscad);
       
       }
      catch(Exception e)
      {pneumaticscad.MessageBox("error", "create element error!");
       return;	  
      }
      if(obj!=null)
       {elementpaste=(tw.com.justiot.pneumatics.pneumaticelement.PneumaticElement) obj;
    ...
    for(int i=0;i<boardLineArray.size();i++)
     ...
     lin=new Line();
      elementpaste=(tw.com.justiot.pneumatics.pneumaticelement.PneumaticElement) tempArray.get(bl.elementi1);
      lin.addPort(elementpaste.ports[bl.porti1]);
      elementpaste=(tw.com.justiot.pneumatics.pneumaticelement.PneumaticElement) tempArray.get(bl.elementi2);
      lin.addPort(elementpaste.ports[bl.porti2]);
      tempLineArray.add(lin);
      lineArray.add(lin);
     }
    pneumaticscad.setModified(true);
    pneumaticscad.addCommand(new pasteBoardToCommand(this,modified,oldBoardArray,oldBoardLineArray,tempArray,tempLineArray));
    repaint();    
   }
  ...
  public void copyToBoard()
   {boolean modified=pneumaticscad.getModified();
    ...
    pneumaticscad.setModified(true);
    pneumaticscad.addCommand(new copyToBoardCommand(this,modified,new ArrayList(boardArray),new ArrayList(boardLineArray),oldbArray,oldbLineArray));    
   }
...
*/
  ...
  private class changeFlowIncCommand extends Command
    ...
    public void undo()
     ...
     pneumaticscad.setModified(modified0);
     }
    public void redo()
        ...
        if(option.equals(Config.getString("PneumaticPanel.deleteline")))
         {if(selectedLine!=null)
           {boolean modified=pneumaticscad.getModified();
            ...
            repaint(); 
            pneumaticscad.setModified(true);
            pneumaticscad.addCommand(new deleteLineCommand(this,modified,sline,line));
  //          mark(); 
           }          
         }
        else if(option.equals(Config.getString("PneumaticPanel.flowvelocity")))
         {    boolean modified0=pneumaticscad.getModified();
              ...
              pneumaticscad.setModified(true);
              pneumaticscad.addCommand(new changeFlowIncCommand(this,modified0,lineinc,Line.defaultlineinc));            
          }
        else if(option.equals(Config.getString("PneumaticPanel.reset")))
        {    
           reset();      
         }
...
//System.err.println("group move");
  ...
  public void groupDelete()
   {boolean modified0=pneumaticscad.getModified();
    ...
    pneumaticscad.setModified(true);
    pneumaticscad.addCommand(new deleteGroupCommand(this,modified0,garray,lines));             
    deGroup();
 //   mark();
   }   
 ...
 //    if(element instanceof EValve)
  //    {WebLadderCAD.self.removeEValve((EValve) element);
  //    }
     repaint();
     return lines;
    }
   ...
   public void deleteElement(PneumaticElement element)
    {boolean modified=pneumaticscad.getModified();
	 ...
	 pneumaticscad.setModified(true);
     pneumaticscad.addCommand(new deleteCommand(element,modified,lines));
    }
/*   
   ...
   public void portClicked(ConnectEvent ce)   // PneumaticPanel
    {
      ...
      {boolean modified=pneumaticscad.getModified();
       ...
       lineArray.add(line); 
       pneumaticscad.setModified(true);
       pneumaticscad.addCommand(new addLineCommand(ce.getSource(),modified,line));
//if(pneumatics.electrics!=null && pneumatics.electrics.sequence!=null) pneumatics.electrics.sequence.refreshSystemCombo();
//       if(circuitListener!=null) 
        {//WebLadderCAD.self.refreshSystemCombo();
         pneumaticscad.setModified(true);
        }
       line=null;       
       started=false;
   ...
   private void moveElementTo(PneumaticElement element,int x,int y)
    {Component com=inComponent(x,y,x+element.getSize().width,y+element.getSize().height);
     if(com!=null && com!=element) 
      {
       if(((tw.com.justiot.pneumatics.pneumaticelement.PneumaticElement)com).getModelType().equals("Actuator") && 
...
//         modelName.equals("LimitSwitch"))
        {Actuator act=(Actuator) com;
         ...
         pneumaticscad.setStatus(Config.getString("Status.install_LS"));      
        } 
       else
    	 pneumaticscad.setStatus(Config.getString("Status.overlay"));
//      return;
      }
...
//
/*  
   public Component[] getPneumaticPanelComponents()  {return WebLadderCAD.self.getPneumaticPanelComponents();}
   public void AddCell(int type,int state,CEDevice ed,CDOutput cdo,boolean ShiftEnabled)
    ...
    WebLadderCAD.self.setModified(true);
    }
   public void setModified(boolean b)
    {WebLadderCAD.self.setModified(b);}
